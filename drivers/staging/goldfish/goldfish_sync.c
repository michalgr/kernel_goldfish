...
*goldfish_fence_parent(struct fence *fence)
{
	return container_of(fence->lock, struct goldfish_sync_timeline, lock);
}

static struct sync_pt *goldfish_sync_fence_to_sync_pt(struct fence *fence)
{
				   ...
				   unsigned int inc)
{
	...
	list_for_each_entry_safe(pt, next, &tl->active_list_head, active_list) {
		/* fence_is_signaled_locked has side effects */
...
static const char
*goldfish_sync_timeline_fence_get_driver_name(struct fence *fence)
{
	return "sw_sync";
}

static const char
*goldfish_sync_timeline_fence_get_timeline_name(struct fence *fence)
{
	struct goldfish_sync_timeline *tl = goldfish_fence_parent(fence);
...
static bool
goldfish_sync_timeline_fence_enable_signaling(struct fence *fence)
{
	struct sync_pt *pt;
	struct goldfish_sync_timeline *tl;
	...
	tl = goldfish_fence_parent(fence);
...
goldfish_sync_fence_create(struct goldfish_sync_timeline *tl, u32 val)
{
    ...
    if (!sync_obj)
    {
        printk(KERN_ERR "Could not create sync fence!"
