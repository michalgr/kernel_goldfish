...
#define AS_DPRINT(fmt,...) printk(KERN_ERR "%s:%d " fmt "\n", __func__, __LINE__, ##__VA_ARGS__);
...
static void
as_ping_impl(struct as_device_state *state, u32 handle)
{
...
static int as_open(struct inode *inode, struct file *filp)
{
	...
	AS_DPRINT("Got free page: %p 0x%llx", ping_info, (unsigned long long)ping_info_phys);
	...
	ping_info_phys_returned =
		((u64)as_read_register(device_state->io_registers, AS_REGISTER_PING_INFO_ADDR_LOW)) |
		((u64)as_read_register(device_state->io_registers, AS_REGISTER_PING_INFO_ADDR_HIGH) << 32);
	AS_DPRINT("Read back");
...
static int as_release(struct inode *inode, struct file *filp)
{
	...
	struct as_allocated_blocks *allocated_blocks = &file_state->allocated_blocks;
