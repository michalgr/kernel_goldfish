...
static int cpufreq_stats_cpu_callback(struct notifier_block *nfb,
...
static struct notifier_block __refdata goldfish_cpufreq_stats_cpu_notifier = {
	.notifier_call = cpufreq_stats_cpu_callback,
};
...
static int __init goldfish_cpufreq_stats_init(void)
{
	...
	register_hotcpu_notifier(&goldfish_cpufreq_stats_cpu_notifier);
...
static void __exit goldfish_cpufreq_stats_exit(void)
{
	...
	unregister_hotcpu_notifier(&goldfish_cpufreq_stats_cpu_notifier);
	for_each_online_cpu(cpu)
		cpufreq_stats_free(cpu);
}
